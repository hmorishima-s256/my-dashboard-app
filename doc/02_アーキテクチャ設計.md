...
- `ipc/registerMainIpcHandlers.ts`
  - IPC ハンドラ登録の合成専用
- `ipc/handlers/calendarHandlers.ts`
  - 予定取得 IPC の登録専用
- `ipc/handlers/settingsHandlers.ts`
  - 設定 IPC の登録専用
- `ipc/handlers/taskHandlers.ts`
  - タスク IPC の登録専用
- `ipc/handlers/authHandlers.ts`
  - 認証 IPC の登録専用
- `ipc/handlers/types.ts`
  - IPC 依存契約型の定義専用
...
- Renderer 側 UI 状態
  - 表示対象日（`useDateEditor`）
  - 認証状態（`useAuthController`）
  - 設定状態（`useDashboardSettings`）
  - 予定行状態（`useCalendarRows`）
...
タスク操作は `taskStore` 経由で lowdb を読み書きし、IPC 戻り値で Renderer へ返す。
...
7. `registerMainIpcHandlers(...)`
   - `calendar/settings/task/auth` の登録関数へ委譲
8. Window 作成
